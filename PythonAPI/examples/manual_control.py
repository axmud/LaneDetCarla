...
# Get absolute path to your project root (adjust as needed)
...
def lane_centerline(left_lane, right_lane, num_points=50):
    left_lane = np.asarray(left_lane, dtype=np.float32)
    right_lane = np.asarray(right_lane, dtype=np.float32)

    left_lane = left_lane[np.argsort(left_lane[:, 1])]
    right_lane = right_lane[np.argsort(right_lane[:, 1])]

    y_min = max(left_lane[:, 1].min(), right_lane[:, 1].min())
    y_max = min(left_lane[:, 1].max(), right_lane[:, 1].max())

    ys = np.linspace(y_min, y_max, num_points)
    left_xs = np.interp(ys, left_lane[:, 1], left_lane[:, 0])
    right_xs = np.interp(ys, right_lane[:, 1], right_lane[:, 0])

    center_xs = (left_xs + right_xs) / 2.0
    centerline = np.stack([center_xs, ys], axis=1)
    return centerline

def get_lane_center(lanes, img_width, img_height, y_eval=None):
    """
    lanes: list of Nx2 arrays, each lane = [[x,y], [x,y], ...]
    returns: center_x, center_y, left_x, right_x
    """
    if lanes is None or len(lanes) < 2:
        return None, None, None, None

    if y_eval is None:
        y_eval = int(img_height * 0.85)   # near bottom of image
    ...
    for lane in lanes:
        ...
        # only use lane if it spans y_eval
        if y_eval < ys.min() or y_eval > ys.max():
            continue

        x_at_y = np.interp(y_eval, ys, x_vals)
        xs.append(x_at_y)
    ...
    # choose nearest lane on left and right of image center
    left_candidates = [x for x in xs if x < img_center_x]
    right_candidates = [x for x in xs if x > img_center_x]

    if left_candidates and right_candidates:
        left_x = max(left_candidates)
        right_x = min(right_candidates)
    else:
        # fallback: take two closest lanes to image center
        xs_sorted = sorted(xs, key=lambda x: abs(x - img_center_x))
        left_x, right_x = sorted(xs_sorted[:2])

    center_x = (left_x + right_x) / 2.0
    center_y = y_eval

    return center_x, center_y, left_x, right_x
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if self._is_quit_shortcut(event.key):
                    ...
                    world.camera_manager.toggle_lane_detection()
...
class CameraManager(object):
    ...
    def toggle_lane_detection(self):
        ...
        if self.lane_detection_enabled and self.lane_model is None:
            try:
                # Load the lane detection model
                self.hud.notification('Loading lane detection model...')
                self.lane_model, self.lane_cfg, self.lane_transform = load_model(
                    LANE_DETECTION_MODELS["CLRNet_Tusimple"]
                )
                self.hud.notification('Lane Detection Enabled')
            except Exception as e:
                self.hud.notification(f'Failed to load lane model: {str(e)}')
                self.lane_detection_enabled = False
        else:
            if self.lane_detection_enabled:
                self.hud.notification('Lane Detection Enabled')
            else:
                self.hud.notification('Lane Detection Disabled')
    ...
    def _parse_image(weak_self, image):
        ...
        if self.sensors[self.index][0].startswith('sensor.lidar'):
            ...
            # Apply lane detection if enabled
            if self.lane_detection_enabled and self.lane_model is not None:
                try:
                    # Convert to BGR for OpenCV
                    bgr_array = array[:, :, ::-1].copy()
                    
                    # Detect lanes
                    lanes, processed_frame = detect_lanes(
                        bgr_array, 
                        self.lane_model, 
                        self.lane_cfg, 
                        self.lane_transform,
                    ...
                    if lanes is not None:
                        ...
                        center_x, center_y, left_x, right_x = get_lane_center(lanes, w, h)

                        if center_x is not None:
                            print(f"Lane center: ({center_x:.1f}, {center_y})")

                            # draw center point
                            cv2.circle(processed_frame, (int(center_x), int(center_y)), 8, (0, 255, 0), -1)

                            # draw image center for reference
                            cv2.circle(processed_frame, (w // 2, int(center_y)), 8, (0, 0, 255), -1)

                            # draw line from vehicle center to lane center
                            cv2.line(
                                processed_frame,
                                (w // 2, int(center_y)),
...
def game_loop(args):
    ...
    try:
        ...
        sim_world = client.load_world('Town06')  # Load a specific map
        if args.sync:
